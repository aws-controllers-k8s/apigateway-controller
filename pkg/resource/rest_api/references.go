...
func (rm *resourceManager) EnsureReferences(
	...
	if desiredKO.Spec.EndpointConfiguration != nil {
		if len(desiredKO.Spec.EndpointConfiguration.VPCEndpointRefs) > 0 {
			if latestKO.Spec.EndpointConfiguration == nil {
				latestKO.Spec.EndpointConfiguration = &svcapitypes.EndpointConfiguration{}
			}
			if len(latestKO.Spec.EndpointConfiguration.VPCEndpointRefs) == 0 {
				latestKO.Spec.EndpointConfiguration.VPCEndpointRefs = desiredKO.Spec.EndpointConfiguration.VPCEndpointRefs
			}
		}
	}
