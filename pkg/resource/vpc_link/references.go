...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForTargetARNs(ctx, apiReader, ko); err != nil {
...
func validateReferenceFields(ko *svcapitypes.VPCLink) error {
	...
	if len(ko.Spec.TargetRefs) > 0 && len(ko.Spec.TargetARNs) > 0 {
		return ackerr.ResourceReferenceAndIDNotSupportedFor("TargetARNs", "TargetRefs")
	}
	if len(ko.Spec.TargetRefs) == 0 && len(ko.Spec.TargetARNs) == 0 {
		return ackerr.ResourceReferenceOrIDRequiredFor("TargetARNs", "TargetRefs")
	}
	return nil
}

// resolveReferenceForTargetARNs reads the resource referenced
// from TargetRefs field and sets the TargetARNs
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForTargetARNs(
	ctx context.Context,
	apiReader client.Reader,
	ko *svcapitypes.VPCLink,
) (hasReferences bool, err error) {
	for _, f0iter := range ko.Spec.TargetRefs {
		if f0iter != nil && f0iter.From != nil {
			hasReferences = true
			arr := f0iter.From
			if arr.Name == nil || *arr.Name == "" {
				return hasReferences, fmt.Errorf("provided resource reference is nil or empty: TargetRefs")
			}
			...
			if ko.Spec.TargetARNs == nil {
				ko.Spec.TargetARNs = make([]*string, 0, 1)
			}
			ko.Spec.TargetARNs = append(ko.Spec.TargetARNs, (*string)(obj.Status.ACKResourceMetadata.ARN))
		}
	}
